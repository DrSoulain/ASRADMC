    ...
    if display:
        ...
        if rnuc != 0:
            plt.contour(np.flipud(im_dist), extent=extent, origin='lower',
                        levels=np.array([rnuc_au]), linewidths=1,
                        alpha=.7, linestyles='--', colors=["red"])
            plt.plot(0, 0, 'r--', label=r'r$_{nuc}$ = %2.1f AU' % rnuc_au)
        plt.contour(np.flipud(im_dist), extent=extent, origin='lower',
...
def ramses_gas_to_dust(ramses_model, param, unit='AU', verbose=False,
                       display=False):
    """ Convert 3D gas grid generated by RAMSES to the dust grid using
    ...
    rho_dust, rr_mas = _apply_sublimation_threshold(rho_dust, rnuc, 
                                                    ramses_model)
    ...
    pstar1, pstar2 = _visu_slice_radmc_input(rho_dust, stellar_pos, rr_mas, fov, dpc,
                                             rnuc=rnuc, unit=unit, display=display)
    ...
    gridinfo = dict2class({'nx': npts, 'ny': npts, 'nz': npts,
                           'xi': xi, 'yi': yi, 'zi': zi,
...
def _compute_star_position(dict_simu):
    """ Retrieve the star position in the simulation 
    in pixels. """
    fov = dict_simu['info']['fov'] / 1000.  # [arcsec]
    npts = dict_simu['info']['npts']
