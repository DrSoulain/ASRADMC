...
class radmc3dOctree(object):
    ...
    def makeWavelengthGrid(self, wbound=None, nw=None, ppar=None):
        ...
        if wbound is None:
            raise ValueError(
                'Unknown wbound. Wavelength grid bondaries are not specified.')
        if nw is None:
            raise ValueError(
                'Unknown nw. Number of wavelength grid points are not specified.')

        self.nwav = nw[0]
        self.wav = wbound[0] * (wbound[1] / wbound[0]
                                )**(np.arange(nw[0], dtype=np.float64) / nw[0])
    ...
    def resolveNodes(self, rsIDs=None):
        ...
        if self.nChild == 8:
            xc_offset = np.array(
                [-0.5, 0.5, -0.5, 0.5, -0.5, 0.5, -0.5, 0.5], dtype=np.float64) * self.dx[rsIDs][0]
            yc_offset = np.array(
                [-0.5, -0.5, 0.5, 0.5, -0.5, -0.5, 0.5, 0.5], dtype=np.float64) * self.dy[rsIDs][0]
            zc_offset = np.array(
                [-0.5, -0.5, -0.5, -0.5, 0.5, 0.5, 0.5, 0.5], dtype=np.float64) * self.dz[rsIDs][0]
        ...
        #
            ...
            if self.act_dim[0] == 0:
                ...
                yc_offset = np.array(
                    [-0.5, 0.5, -0.5, 0.5], dtype=np.float64) * self.dy[rsIDs][0]
                zc_offset = np.array(
                    [-0.5, -0.5, 0.5, 0.5], dtype=np.float64) * self.dz[rsIDs][0]
            elif self.act_dim[1] == 0:
                xc_offset = np.array(
                    [-0.5, 0.5, -0.5, 0.5], dtype=np.float64) * self.dx[rsIDs][0]
        ...
        #
            ...
            if self.act_dim[0] == 1:
                xc_offset = np.array(
                    [-0.5, 0.5], dtype=np.float64) * self.dx[rsIDs][0]
            ...
            if self.act_dim[1] == 1:
                ...
                yc_offset = np.array(
                    [-0.5, 0.5], dtype=np.float64) * self.dy[rsIDs][0]
                zc_offset = np.array([0., 0.], dtype=np.float64)
            ...
            if self.act_dim[2] == 1:
                ...
                yc_offset = np.array([0., 0.], dtype=np.float64)
                zc_offset = np.array(
                    [-0.5, 0.5], dtype=np.float64) * self.dz[rsIDs][0]
        ...
        for i in range(self.nChild):
            ...
            if self.act_dim[0] == 1:
                dx[ind + i] = np.zeros(ncell, dtype=np.float64) + \
                    self.dx[rsIDs][0] * 0.5
            else:
                dx[ind + i] = np.zeros(ncell,
                                       dtype=np.float64) + self.dx[rsIDs][0]

            if self.act_dim[1] == 1:
                dy[ind + i] = np.zeros(ncell, dtype=np.float64) + \
                    self.dy[rsIDs][0] * 0.5
            else:
                dy[ind + i] = np.zeros(ncell,
                                       dtype=np.float64) + self.dy[rsIDs][0]

            if self.act_dim[2] == 1:
                dz[ind + i] = np.zeros(ncell, dtype=np.float64) + \
                    self.dz[rsIDs][0] * 0.5
            else:
                dz[ind + i] = np.zeros(ncell,
                                       dtype=np.float64) + self.dz[rsIDs][0]

            isLeaf[ind + i] = np.ones(ncell, dtype=bool)
            level[ind + i] = np.zeros(ncell, dtype=np.int) + \
                self.level[rsIDs][0] + 1
    ...
    def makeSpatialGrid(self, ppar=None, levelMaxLimit=None, dfunc=None, model='', **kwargs):
        ...
        if dfunc is None:
            fnamelist = [f[0] for f in inspect.getmembers(
                self.model) if inspect.isfunction(f[1])]
    ...
    def selfCheck(self):
        ...
        print('Nr of leaves    : ' + ("%d" %
                                      self.counter[0]) + " should be " + ("%d" % self.nLeaf))
        print('Nr of branches  : ' + ("%d" %
                                      self.counter[1]) + " should be " + ("%d" % self.nBranch))
        print('Nr of cells     : ' + ("%d" % self.nCell) +
              " should be " + ("%d" % (self.nBranch + self.nLeaf)))
    ...
    def convArrLeaf2Tree(self, var=None):
        ...
        if ndim == 1:
            ...
            treeVar = np.zeros(
                [self.nLeaf + self.nBranch, var.shape[1]], dtype=var.dtype)
            ...
            assert (
                ndim < 1), 'Incorrect shape for input array. Input array has too few dimensions (i.e. < 1). '
    ...
    def writeSpatialGrid(self, fname=''):
        ...
        with open(fname, 'w') as wfile:
            ...
            # AMR self.style (0=regular, 1 - Octree, 10 - Layered)
            wfile.write('%d\n' % 1)
            if self.crd_sys == 'car':
    ...
    def _writeOcTreeNodeTypeRec(self, cellID=None, wfile=None):
        ...
        if self.isLeaf[cellID]:
            ...
            for i in range(self.childID[cellID].shape[0]):
                self._writeOcTreeNodeTypeRec(
                    cellID=self.childID[cellID][i], wfile=wfile)
    ...
    def readSpatialGrid(self, fname=''):
        ...
        with open(fname, 'r') as rfile:
            ...
            if grid_style != 1:
                raise ValueError('Unsupported AMR style in ' +
                                 fname + '. Current only Octree AMR is supported.')
            ...
            if 1 in self.act_dim:
                self.nChild = 2**(np.array(self.act_dim, dtype=int).sum())
            dum = rfile.readline().split()
            self.nxRoot, self.nyRoot, self.nzRoot = int(
                dum[0]), int(dum[1]), int(dum[2])
            ...
            self.xc = (self.xi[0:self.nxRoot] +
                       self.xi[1:self.nxRoot + 1]) * 0.5
            self.yc = (self.yi[0:self.nyRoot] +
                       self.yi[1:self.nyRoot + 1]) * 0.5
            self.zc = (self.zi[0:self.nzRoot] +
                       self.zi[1:self.nzRoot + 1]) * 0.5
    ...
    def _readGridNodeTypeOcTreeRec(self, cellID=None, rfile=None):
        ...
        if dum == 1:
            ...
            if self.nChild == 8:
                xc_offset = np.array(
                    [-0.5, 0.5, -0.5, 0.5, -0.5, 0.5, -0.5, 0.5], dtype=np.float64) * self.dx[cellID]
                yc_offset = np.array(
                    [-0.5, -0.5, 0.5, 0.5, -0.5, -0.5, 0.5, 0.5], dtype=np.float64) * self.dy[cellID]
                zc_offset = np.array(
                    [-0.5, -0.5, -0.5, -0.5, 0.5, 0.5, 0.5, 0.5], dtype=np.float64) * self.dz[cellID]
            #
            # If we are resolving only two dimensions
            #
            elif self.nChild == 4:
                ...
                if self.act_dim[0] == 0:
                    xc_offset = np.array([0., 0., 0., 0.], dtype=np.float64)
                    yc_offset = np.array(
                        [-0.5, 0.5, -0.5, 0.5], dtype=np.float64) * self.dy[cellID]
                    zc_offset = np.array(
                        [-0.5, -0.5, 0.5, 0.5], dtype=np.float64) * self.dz[cellID]
                elif self.act_dim[1] == 0:
                    xc_offset = np.array(
                        [-0.5, 0.5, -0.5, 0.5], dtype=np.float64) * self.dx[cellID]
                    yc_offset = np.array([0., 0., 0., 0.], dtype=np.float64)
                    zc_offset = np.array(
                        [-0.5, -0.5, 0.5, 0.5], dtype=np.float64) * self.dz[cellID]
                elif self.act_dim[2] == 0:
            ...
            #
                ...
                if self.act_dim[0] == 1:
                    xc_offset = np.array(
                        [-0.5, 0.5], dtype=np.float64) * self.dx[cellID]
                    yc_offset = np.array([0., 0.], dtype=np.float64)
                    zc_offset = np.array([0., 0.], dtype=np.float64)
                ...
                if self.act_dim[1] == 1:
                    xc_offset = np.array([0., 0.], dtype=np.float64)
                    yc_offset = np.array(
                        [-0.5, 0.5], dtype=np.float64) * self.dy[cellID]
                    zc_offset = np.array([0., 0.], dtype=np.float64)
                ...
                if self.act_dim[2] == 1:
                    ...
                    yc_offset = np.array([0., 0.], dtype=np.float64)
                    zc_offset = np.array(
                        [-0.5, 0.5], dtype=np.float64) * self.dz[cellID]

            else:
                raise ValueError('Wrong number of child/leaf cells. The number of leaf cells should be 2,4,8 in '
                                 + '1,2,3 dimensions, respectively. Insted ' +
                                 ("%d" % self.nChild) + ' have been '
                                 + 'claimed.')
            ...
            for i in range(self.nChild):
                ...
                self.putNode(
                    crd=(self.x[cellID] + xc_offset[i], self.y[cellID] +
                         yc_offset[i], self.z[cellID] + zc_offset[i]),
                    cellsize=(dx, dy, dz), level=self.level[cellID] + 1,
                    parentID=cellID, cellID=self.cellIDCur)
                self.cellIDCur += 1
                self._readGridNodeTypeOcTreeRec(
                    cellID=self.childID[cellID][i], rfile=rfile)
